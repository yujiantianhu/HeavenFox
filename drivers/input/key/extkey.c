...
/*!< API function */
/*!
 * @brief   extern key irq handler
 * @param   irq, args (struct extkey_drv_data *)
 * @retval  irq retval
 * @note    none
 */
static irq_return_t extkey_driver_isr(kint32_t irq, void *args)
{
	struct extkey_drv_data *sptr_data;

	sptr_data = (struct extkey_drv_data *)args;
...
static kssize_t extkey_driver_read(struct fwk_file *sptr_file, kbuffer_t *ptrBuffer, kssize_t size)
{
	...
	if (!(sptr_file->mode & O_NONBLOCK))
		wait_event_timeout(&sptr_data->sgtc_wqh, sptr_data->wake, 20U);
	else
	{
		if (!sptr_data->wake)
			return -ER_NREADY;
