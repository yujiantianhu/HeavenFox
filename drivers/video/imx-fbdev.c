 ...
 * @note    none
 */
static void imx_fbdev_backlight_enable(struct imx_fbdev_drv *sprt_drv)
{
    srt_hal_imx_pin_t sgrt_cfg;

    if (!sprt_drv->sgrt_blight.sprt_gdesc)
        return;

    hal_imx_pin_attribute_init(&sgrt_cfg, IMX6UL_PIN_ADDR_BASE, IMX6UL_MUX_GPIO1_IO08_GPIO1_IO08, 0, 0);

    /*!< Soft Start */
    for (kint32_t i = 0; i < 5; i++)
    {
        mrt_clrbitl(IMX6UL_IO_CTL_PAD_DSE_BIT(IMX6UL_IO_CTL_PAD_DSE_MASK), &sgrt_cfg.pad_base);
        mrt_setbitl(IMX6UL_IO_CTL_PAD_DSE_BIT(IMX6UL_IO_CTL_PAD_DSE_RDIV(i + 1)), &sgrt_cfg.pad_base);
    ...
    mrt_clrbitl(IMX6UL_IO_CTL_PAD_DSE_BIT(IMX6UL_IO_CTL_PAD_DSE_MASK), &sgrt_cfg.pad_base);
    mrt_setbitl(IMX6UL_IO_CTL_PAD_DSE_BIT(IMX6UL_IO_CTL_PAD_DSE_RDIV(6)), &sgrt_cfg.pad_base);
    fwk_gpio_set_value(sprt_drv->sgrt_blight.sprt_gdesc, 1);
}
 ...
 * @note    none
 */
#define imx_fbdev_backlight_disable(sprt_drv)   \
    do {    \
        if ((sprt_drv)->sgrt_blight.sprt_gdesc)   \
            fwk_gpio_set_value((sprt_drv)->sgrt_blight.sprt_gdesc, 0);    \
    } while (0)
 ...
 * @note    none
 */
static void imx_fbdev_wake_interface(struct imx_fbdev_drv *sprt_drv)
{
    struct fwk_fb_notifier_param *sprt_param;

    switch (sprt_drv->interface_type)
    {
        case NR_IMX_FBDEV_ELCDIF:
            /*!< open backlight */
            imx_fbdev_backlight_enable(sprt_drv);
            break;
        
        case NR_IMX_FBDEV_HDMI:
            sprt_param = kmalloc(sizeof(*sprt_param), GFP_KERNEL);
            if (!isValid(sprt_param))
                return;

            sprt_param->sprt_fix = &sprt_drv->sprt_fb->sgrt_fix;
            sprt_param->sprt_var = &sprt_drv->sprt_fb->sgrt_var;
            sprt_param->bus_width = sprt_drv->sgrt_phase.bus_width;

            fwk_blocking_notifier_call_chain(
                &sgrt_fbmem_notifier_chain, FB_NOTIFIER_HDMI_OPEN, sprt_param);
 ...
 * @note    none
 */
static void imx_fbdev_quit_interface(struct imx_fbdev_drv *sprt_drv)
{
    switch (sprt_drv->interface_type)
    {
        case NR_IMX_FBDEV_ELCDIF:
            /*!< open backlight */
            imx_fbdev_backlight_disable(sprt_drv);
            break;
        
        case NR_IMX_FBDEV_HDMI:
            fwk_blocking_notifier_call_chain(
                &sgrt_fbmem_notifier_chain, FB_NOTIFIER_HDMI_CLOSE, mrt_nullptr);
            break;
...
static kint32_t imx_fbdev_open(struct fwk_fb_info *sprt_info, kint32_t user)
{
    ...
    srt_imx_lcdif_t *sprt_lcdif;

    sprt_drv = fwk_fb_get_drvdata(sprt_info);
    sprt_lcdif = (srt_imx_lcdif_t *)sprt_drv->base;

    /*!< Enable interface */
    imx_fbdev_wake_interface(sprt_drv);
...
static kint32_t imx_fbdev_close(struct fwk_fb_info *sprt_info, kint32_t user)
{
    ...
    /*!< Close Interface */
    imx_fbdev_quit_interface(sprt_drv);

    /*!< Disable LCD */
    mrt_writel(0U, &sprt_lcdif->CTRL_SET);
 ...
 * @note    none
 */
kint32_t imx_fbdev_ioctl(struct fwk_fb_info *sprt_info, kuint32_t cmd, kuaddr_t arg)
{
    struct fwk_fb_var_screen_info *sprt_var;
    struct imx_fbdev_drv *sprt_drv;
    srt_imx_lcdif_t *sprt_lcdif;
    ...
    switch (cmd)
    {
        case NR_FB_IOSET_VARINFO:
            ...
            /*!< CUR_BUF: Address of the current frame being transmitted by eLCDIF. */
            mrt_writel(new_smem, &sprt_lcdif->CUR_BUF);
            /*!< NEXT_BUF: Address of the next frame that will be transmitted by eLCDIF */
            mrt_writel(new_smem, &sprt_lcdif->NEXT_BUF);
...
static kint32_t imx_fbdev_driver_probe_timings(struct fwk_platdev *sprt_pdev)
{
    ...
    if (fwk_of_find_property(sprt_disp, "hdmi-endpoint", mrt_nullptr))
        sprt_drv->interface_type = NR_IMX_FBDEV_HDMI;
    else
    {
...
fail7:
    kfree(buffer);
fail6:
    if (sprt_drv->interface_type == NR_IMX_FBDEV_ELCDIF)
