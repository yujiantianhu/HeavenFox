...
/*!< The defines */
typedef ksint32_t (*func_hal_irq_init_cb_t) (struct hal_device_node *, struct hal_device_node *);
 ...
 * @note    none
 */
void hal_of_irq_init(const struct hal_of_device_id *sprt_matches)
{
    struct hal_irq_intcs_desc
    {
    ...
    struct hal_irq_intcs_desc *sprt_desc, *sprt_desc_temp;
    struct hal_of_device_id *sprt_match;
    struct hal_device_node *parent = mrt_nullptr;
    kbool_t of_status;
    ksint32_t retval;
    ...
    {
        sprt_prop = hal_of_find_property(sprt_np, "interrupt-controller", mrt_nullptr);
        of_status = hal_of_device_is_avaliable(sprt_np);
        ...
        if (!hal_of_node_try_matches(sprt_np, sprt_matches, mrt_nullptr))
        {
            continue;
        }

        /*!< matched sucessfully */
        sprt_desc = (struct hal_irq_intcs_desc *)kzalloc(sizeof(struct hal_irq_intcs_desc), GFP_KERNEL);
        ...
        sprt_desc->sprt_parent = hal_of_irq_parent(sprt_np);
    ...
    while (!mrt_list_head_empty(&sgrt_intc_desc_list))
    {
        ...
        {
            sprt_match = hal_of_match_node(sprt_matches, sprt_desc->sprt_np);
            ...
            /*!< do intc initial */
            retval = ((func_hal_irq_init_cb_t)sprt_match->data)(sprt_desc->sprt_np, sprt_desc->sprt_parent);
            if (mrt_isErr(retval))
            {
