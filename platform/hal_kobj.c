...
/*!< The globals */
struct hal_kobj_map *sprt_hal_chrdev_map;
struct hal_kobj_map *sprt_hal_blkdev_map;
struct hal_kobj_map *sprt_hal_netdev_map;
 ...
 * @note    none
 */
ksint32_t __plat_init hal_kobj_init(void)
{
	kuint32_t i;

	sprt_hal_chrdev_map = (struct hal_kobj_map *)kzalloc(sizeof(struct hal_kobj_map), GFP_KERNEL);
	if (!mrt_isValid(sprt_hal_chrdev_map))
	{
		goto fail1;
	}

	sprt_hal_blkdev_map = (struct hal_kobj_map *)kzalloc(sizeof(struct hal_kobj_map), GFP_KERNEL);
	if (!mrt_isValid(sprt_hal_blkdev_map))
	{
		goto fail2;
	}

	sprt_hal_netdev_map = (struct hal_kobj_map *)kzalloc(sizeof(struct hal_kobj_map), GFP_KERNEL);
	if (!mrt_isValid(sprt_hal_netdev_map))
	{
		goto fail3;
	}

	/*!< Initialize the character device matching table */
	for (i = 0; i < ARRAY_SIZE(sprt_hal_chrdev_map->sprt_probes); i++)
	{
		sprt_hal_chrdev_map->sprt_probes[i] = mrt_nullptr;
	}

	/*!< Initialize the block device matching table */
	for (i = 0; i < ARRAY_SIZE(sprt_hal_blkdev_map->sprt_probes); i++)
	{
		sprt_hal_blkdev_map->sprt_probes[i] = mrt_nullptr;
	}

	/*!< Initialize the net device matching table */
	for (i = 0; i < ARRAY_SIZE(sprt_hal_netdev_map->sprt_probes); i++)
	{
		sprt_hal_netdev_map->sprt_probes[i] = mrt_nullptr;
	}

	return mrt_retval(Ert_isWell);

fail3:
	kfree(sprt_hal_blkdev_map);
	sprt_hal_blkdev_map = mrt_nullptr;
fail2:
	kfree(sprt_hal_chrdev_map);
	sprt_hal_chrdev_map = mrt_nullptr;
fail1:
	return mrt_retval(Ert_isMemErr);
}
IMPORT_PLATFORM_INIT(hal_kobj_init);
 ...
 * @note    none
 */
void __plat_exit hal_kobj_del(void)
{
	struct hal_probes *sprt_prev;
	struct hal_probes *sprt_list;
	kusize_t mapsize;
	kuint32_t i;

	/*!< Destroy the character device matching table */
	mapsize	= mrt_isValid(sprt_hal_chrdev_map) ? ARRAY_SIZE(sprt_hal_chrdev_map->sprt_probes) : 0;
	for (i = 0; i < mapsize; i++)
	{
		mrt_list_delete_all(sprt_hal_chrdev_map->sprt_probes[i],
										sprt_prev, sprt_list);
		sprt_hal_chrdev_map->sprt_probes[i] = mrt_nullptr;
	}
	kfree(sprt_hal_chrdev_map);
	...
	mapsize	= mrt_isValid(sprt_hal_blkdev_map) ? ARRAY_SIZE(sprt_hal_blkdev_map->sprt_probes) : 0;
	for (i = 0; i < mapsize; i++)
	{
		mrt_list_delete_all(sprt_hal_blkdev_map->sprt_probes[i],
										sprt_prev, sprt_list);
		sprt_hal_blkdev_map->sprt_probes[i] = mrt_nullptr;
	}
	kfree(sprt_hal_blkdev_map);
	...
	mapsize	= mrt_isValid(sprt_hal_netdev_map) ? ARRAY_SIZE(sprt_hal_netdev_map->sprt_probes) : 0;
	for (i = 0; i < mapsize; i++)
	{
		mrt_list_delete_all(sprt_hal_netdev_map->sprt_probes[i],
										sprt_prev, sprt_list);
		sprt_hal_netdev_map->sprt_probes[i] = mrt_nullptr;
	}
	kfree(sprt_hal_netdev_map);
	sprt_hal_netdev_map = mrt_nullptr;
}
IMPORT_PLATFORM_EXIT(hal_kobj_del);
 ...
 * @note    none
 */
ksint32_t hal_kobj_map(struct hal_kobj_map *domain, kuint32_t devNum, kuint32_t range, void *data)
{
	struct hal_probes *sprt_probe;
	kuint32_t major;
	kuint32_t majorCnt;
	kuint32_t i;
	kusize_t probeMax;
	...
	sprt_probe = (struct hal_probes *)kzalloc(sizeof(struct hal_probes) * majorCnt, GFP_KERNEL);
	...
	{
		struct hal_probes *sprt_Temp;
		struct hal_probes **sprt_Dst;
		kuint32_t index;
		 ...
		 * 		1) kobj doesn't need to worry about invalid or duplicate device numbers, as this step has already been done in the hal_register_chrdev
		 ...
		 * 			hal_register_chrdev have ensured that this range of device numbers cannot be illegal!
		 */
		sprt_Temp->data = data;
...
/*!
 * @brief   hal_kobj_unmap
 * @param   none
 * @retval  none
 * @note    none
 */
ksint32_t hal_kobj_unmap(struct hal_kobj_map *domain, kuint32_t devNum, kuint32_t range)
{
	struct hal_probes *sprt_Rlt;
	kuint32_t major;
	kuint32_t majorCnt;
	kuint32_t i;
	kusize_t probeMax;
...
/*!
 * @brief   hal_kobj_lookUp
 * @param   none
 * @retval  none
 * @note    none
 */
void *hal_kobj_lookUp(struct hal_kobj_map *domain, kuint32_t devNum)
{
	struct hal_probes *sprt_Temp;
	kuint32_t index;
	void *data;
