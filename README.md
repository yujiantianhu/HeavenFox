<h1 align="center"> HeavenFox OS</h1>
<p align="center">HeavenFox OS is a lightweight embedded system kernel similar to Linux</p>

**English** | [中文](./README_zh.md)

![heavenfox](lib/rootfs/boot/windows/fox1080p.jpg)

-------------------------------------------------------------------------

#### Catalog
- [Note](#note)
- [Information](#information)
- [disclaimer](#disclaimer)
- [Feature](#feature)
- [Methods](#methods)
- [Folder Instruction](#folder-instruction)
- [Updating Logs](#updating-logs)

## Note
    HeavenFox OS aims to simplify the Linux kernel by refering to the kernel mechanism,
    which is convenient for the developers to research and apply, and the project follows
    the GPL license and is open source for life.

## Information
    author:     Yang Yujun (杨雨俊)
    created on: 2023.12.21
    e-mail:     <yujiantianhu@163.com>
    git:        git@github.com:yujiantianhu/HeavenFox.git

## disclaimer
    Night-Rain.Yang, talented-rain and yujiantinahu are all the author's own (Yang Yujun)

## Feature
    01) support multithreading (time slice, priority preemption, and delayed sleep);
    02) support initcall implicit initialization;
    03) support memory pool;
    04) support list, ring queue/buffer, radix tree, rb-tree;
    05) suuport automatic generation of link script to adapt to different CPU configurations;
    06) support printk (print interface), allowing kernel asynchronous printing (reading ring buffer by "kthread" task);
    07) support character device driver framework;
    08) support framebuffer driver framework;
    09) support IRQ virtual interrupt numbers mapping (GIC_SPI);
    10) support timer link list (timer_list);
    11) support SD Card (porting fatfs), and provide general APIs, such as file_open/file_close, ...;
    12) support Device Tree (support reading .dtb in SD Card, and parsing it to device_node and platform_device);
    13) support platform: bus - device - driver framework;
    14) provide APIs such as open/close/read/write, access hardware through "fd" and "file_operations";
    15) support flexible selection of source files to be complied through Makefile;
    16) support notifier chain, drivers can access other drivers by registering and invoking the notifier chain;
    17) support pinctrl, gpio (and gpio-irq), clock subsystem;
    18) support i2c bus framework: adapter - client - driver;
    19) support workqueue, mutex lock, spin lock, waitqueue, semaphore, read-write lock and mailbox (inter-thread communication, e.g, ITC);
    20) support priority inherit for mutex and write lock;
    21) support DTC compiler, dtb can be generated by "make dtbs";
    22) support virtual file system (kset, kobject and inode) and operations, such as virt_open/virt_close, ...;
    23) support touchscreen driver of "tsc2007", environment sensor driver "ap3216c", eeprom driver "at24c02";
    24) support cpu zynq7 (boot and running);
    25) support configure resources freely by "configs/mach/xxxx_defconfig', and generate auto.conf and autoconf.h;
    26) support lvgl;
    27) suuport lwip (code packaging: network drivers and application layer commuicate with sk_buff and net_device);
    28) support terminal, provide system command "help", "ts", "ttc", "ping" and so on;
    
## Methods
    make:           build the whole project;
    make all:       the same as "make"
    make dtbs:      build device-tree only;
    make clean:     delete all the .o and .d;
    make distclean: delete the ./objects/* and .elf, .dis, .img and so on;
    make config:    generate auto.conf and autoconf.h, maybe you can input: make CONFIGS=xxxx_defconfig config
    make info:      get this project information;
    make libs:      build "lib/" only;
    make local:     build but not include dtbs and libs
    
    or using cmake:
    mkdir -p ./build
    cd ./build
    make distclean
    cmake -DCMAKE_BUILD_TYPE=debug ..
    make
    
    Image path: ./boot/image/HeavenFox.img
    Dtbs  path: ./boot/image/firmware.dtb
    
## Folder Instruction
    boot/:          containing device-tree, memory resources and OS image;
    configs/:       optional feature configurations (by setting macro definitions);
    build/:         for CMake (perhaps you can create it by "mkdir -p ./build";
    document/:      instuction documents;
    scripts/:       some tools for compiling or debugging, such as dtc, jlink scripts, ..., and Makefile.build;
    objects/:       when you build the project, the generated intermediate files will be stored here;
    
    arch/:          cpu architecture related files, contain chip-startup, vector table and register definitions for each cpu, ...;
    board/:         specific board related files, contain direct operations on board resources;
    common/:        the generic definitions and functions, which are applied to all files;
    drivers/:       the hardware drivers for each chip and board perephral;
    example/:       application instances;
    fs/:            file system related files
    include/:       headers for the whole project;
    init/:          global initialization, such as "main.c";
    kernel/:        for multi-thread features, containing scheduler, and generic threading methods;
    lib/:           the third-party libraries or code;
    platform/:      the generic abstract framework for system operations, provides methods to standardize programming specifications;
    term/:          terminal console and command interface

## Updating Logs
- 2025.07.20
```
    author: Yang Yujun
    commit: 
        1) optimize code (fix bug);
        2) merge branches (support notifier, sil9022a driver, more terminal commands, application examples, ...)
```

- 2024.12.21
```
    author: Yang Yujun
    commit: add network platform interface, support lwip libraries, and ARP, IP (UDP) loopback
```

- 2024.11.17
```
    author: Yang Yujun
    commit: 
        1) add third-party library "lvgl";
        2) adjust the compilation method of third-party libs (linked to main project with static libraries)
```

- 2024.11.09
```
    author: Yang Yujun
    commit: 
        1) fix the problem where the context can not be restored after thread was switched;
        2) support cpu xc7z010, including uart, hdmi, ...;
        3) support configure resources freely by "configs/mach/xxxx_defconfig', and generate auto.conf and autoconf.h;
        4) rename directory "rootfs" > "fs", provide general file system APIs (file_open/file_write/...);
        5) support block device and gendisk simply;
        6) add new directory "lib", the third-party libraries and projects will be put here;
        7) framebuffer can be allocated by "kmalloc", but the argument is GFP_DRAM instead of GFP_NORMAL;
        8) fatfs can be called by file_open, disk will be mounted to virtual file system "/media/FATFS_x";
        9) optimize top Makefile and Makefile.build's structure, fix the issue of duplicate compliation during make;
        10) change the path of image generation
```

- 2024.07.02
```
    author: Yang Yujun
    commit: support cpu "xc7z010" (initial SD Card, and copy dtb to DDR; then booting to "start_kernel")
```

- 2024.06.12
```
    author: Yang Yujun
    commit: 
        1)  support kobject and kset, every device can be populated to virtual file system;
        2)  add i2c client driver: ap3216c, at24c02;
        3)  all implict initializations (platform_init, pattern_init, ...) move to drivers/base
```

- 2024.06.05
```
    author: Yang Yujun
    commit: 
        1)  code maintenance, optimize LCD display code and i2c driver code of imx6ull;
        2)  change the display content of "display-app" thread
```

- 2024.06.03
```
    author: Yang Yujun
    commit: support DTC compiler, dtb can be generated by "make dtbs"
```

- 2024.06.02
```
    author: Yang Yujun
    commit: 
        1)  support semaphore, wait queue;
        2)  standardize the naming of scheduler APIs;
        3)  support multiple scheduling methods (configured by "CONFIG_PREEMPT", "CONFIG_SCHEDUL", ...)
```

- 2024.05.27
```
    author: Yang Yujun
    commit: standardize the naming of error code
```

- 2024.05.26
```
    author: Yang Yujun
    commit:
        1)  add mailbox (inter-thread communication);
        2)  optimize mutex lock and spin lock;
        3)  add workqueue (for the low half interrupt);
        4)  standardize the naming of variable types (such as kint32_t, structure, and so on)
        5)  change project name to "HeavenFox"
```

- 2024.05.13
```
    author: Yang Yujun
    commit:
        1)  add pinctrl device framework;
        2)  add gpio device framework;
        3)  add clock system framework;
        4)  add i2c master, device and instance (tsc2007) driver;
        5)  optimize irq framework
```

- 2024.04.18
```
    author: Yang Yujun
    commit: configure .gitignore, ignore vscode and object
```

- 2024.04.16
```
    author: Yang Yujun
    commit: change project name to "gardens"
```

- 2024.04.15
```
    author: Yang Yujun
    commit: 
        1)  fix the issue that the first thread scheduling register was overwritten;
        2)  add display generic APIs
        3)  optimize code
```

- 2024.04.13
```
    author: Yang Yujun
    commit: makefile optimization, supporting selective compilation of source files
```

- 2024.04.07
```
    author: Yang Yujun
    commit:
        1)  support multithreading (time slice, priority preemption, and delayed sleep);
        2)  support initcall implicit initialization;
        3)  support character device driver framework;
        4)  support framebuffer driver framework;
        5)  support IRQ virtual interrupt numbers mapping (GIC_SPI);
        6)  support timer link list (timer_list);
        7)  support SD Card (porting fatfs);
        8)  support Device Tree (support reading .dtb in SD Card, and parsing it to device_node and platform_device);
        9)  support platform: bus - device - driver framework;
        10) provide APIs such as open/close/read/write, access hardware through “fd” and “file_operations”.
```

- 2023.12.21
```
    author: Yang Yujun
    commit: create README.md first
```
